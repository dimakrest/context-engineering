...
2. **Invoke Gemini CLI to review the plan:**
   ...
   ```bash
   gemini -y "You are a senior software architect reviewing an implementation plan. Be critical and thorough.

   Review the implementation plan at: <plan_path>

   First, read the plan file. Then read these project guidelines if they exist:
   - CLAUDE.md (project rules and principles)
   - Any architecture or standards documentation
   ...
   8. **Security Considerations**: Any security implications?
   ...
   Be constructive but don't hold back."
   ```
...
- Review checks plan against project-specific rules
