...
MODELS = {"claude": "opus", "codex": "gpt-5.4"}
...
def mission_at(path, mode):
    ...
    for name in list(SOURCES) + (["design.md"] if mode == "design" else []):
...
def source_hashes(mission, mode):
    return {name: file_hash(mission / name) for name in list(SOURCES) + (["design.md"] if mode == "design" else [])}
...
def selection(args):
    require(args.author in ("claude", "codex"), "unknown author provider; independence cannot be established")
    reviewer = args.reviewer or ("codex" if args.author == "claude" else "claude")
    require(reviewer in MODELS and reviewer != args.author, "reviewer must be the other known provider")
    model = args.model or MODELS[reviewer]
    if reviewer == "claude":
        valid = re.fullmatch(r"claude-[a-z0-9.-]+", model) or model in ("opus", "sonnet", "haiku", "fable")
    else:
        valid = re.fullmatch(r"(?:gpt-[0-9][a-z0-9.-]*|o[1-9][a-z0-9.-]*)", model)
    require(valid, "model does not identify the selected provider")
...
def preflight(chosen, timeout=15):
    ...
    with tempfile.TemporaryDirectory(prefix="crosscheck-preflight-", dir=scratch_root(chosen.get("repo"))) as cwd:
        ...
        require(bool(re.fullmatch(r"codex-cli \S+", version)) if provider == "codex" else
                bool(re.fullmatch(r"\S+ \(Claude Code\)", version)), "executable provider identity is unknown")
        if provider == "claude":
            help_text = probe([binary, "--help"], cwd, env, timeout)
            flags = ["--print", "--output-format", "--verbose", "--safe-mode", "--setting-sources", "--settings",
                     "--no-session-persistence", "--tools", "--allowedTools", "--strict-mcp-config", "--mcp-config",
                     "--model", "--session-id", "--permission-mode", "--disable-slash-commands"]
        else:
            help_text = probe([binary, "exec", "--help"], cwd, env, timeout)
            flags = ["--json", "--sandbox", "--ephemeral", "--ignore-user-config", "--config", "--model", "--cd", "--skip-git-repo-check"]
        require(all(flag in help_text for flag in flags), "reviewer lacks required transcript/session CLI capabilities")
        if provider == "claude":
            auth = probe([binary, "--safe-mode", "--setting-sources", "", "auth", "status"], cwd, env, timeout)
            try:
            ...
            auth = probe([binary, "login", "status"], cwd, env, timeout)
            require("Logged in using" in auth, "unrecognized Codex authentication status")
...
def command(chosen, cwd, session):
    binary = chosen["executable"]
    if chosen["reviewer"] == "claude":
    ...
    return [binary, "exec", "--cd", str(cwd), "--skip-git-repo-check", "--sandbox", "read-only",
            "--json", "--ephemeral", "--ignore-user-config", "--model", chosen["model"],
            "--config", 'model_provider="openai"', "--config", 'project_doc_max_bytes=0', "-"]


def snapshot(mission, directory):
...
def audit_record(record, mission, repo, identity, kind="blind"):
    ...
    report = claude_report(raw, access, record["session_id"]) if identity["reviewer"] == "claude" else codex_report(raw, access)
...
def void(record, reason, mission, key):
    ...
    usable = directory.is_absolute() and directory.name.startswith("mission-crosscheck-") and directory.is_dir()
    if usable:
        for name in ("transcript.jsonl", "stderr"):
            path = directory / name
            if path.is_file():
                dest = directory / ("VOID-" + name)
                path.rename(dest)
                record.setdefault("quarantine", {})[name] = str(dest)
    report = mission / "crosscheck" / ("pass1-report.md" if key == "blind" else "pass2-report.md")
    if report.is_file() and usable:
        dest = directory / ("VOID-" + report.name)
        shutil.move(str(report), str(dest))
        record.setdefault("quarantine", {})[report.name] = str(dest)


@contextmanager
def run_lock(mission):
    name = "crosscheck-" + digest(str(mission).encode()) + ".lock"
    with open(scratch_root(repo_root(mission)) / name, "a") as lock:
        try:
...
def execute_locked(args, mission, repo):
    ...
    try:
        ...
        require(progress.get("version") == VERSION, "legacy progress lacks completion evidence")
        if args.sighted:
        ...
        changed = False
        for affected in (("blind", "sighted") if args.sighted else (key,)):
            record = progress.get(affected)
            if record and record.get("audit_status") != "VOID":
                void(record, str(exc), mission, affected)
                changed = True
        if changed:
    ...
    if args.sighted:
        ...
        task = f"""# Sighted divergence (not a blind pass)
Read {run_dir / 'SIGHTED-design.md'} and {run_dir / 'BLIND-report.md'}.
Repository: {repo}. Original sealed inputs: {current['package']}.
...
"""
    ...
    snapshot(mission, run_dir / "snapshot")
    ...
    launch = run_dir / "cwd"
    launch.mkdir()
    (run_dir / "cwd" / "TASK.md").write_text(task)
    ...
    try:
        ...
        void(record, str(exc), mission, key)
        save(state_path, progress)
        raise Invalid(str(exc)) from exc


def finish(progress, key, record, report, state_path):
    if record.get("sighted_inputs"):
        for name, expected in record["sighted_inputs"].items():
            require(file_hash(Path(record["run_dir"]) / name) == expected, "sighted evidence changed")
    ...
    destination = state_path.parent / ("pass1-report.md" if key == "blind" else "pass2-report.md")
