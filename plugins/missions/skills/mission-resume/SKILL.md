...
| `crosscheck/progress.json` holds a `PENDING` or `VOID` record | An external review died mid-pass. Rerun the same `crosscheck.py run` command with the package directory it was sealed into: a completed but unaudited run is audited without another reviewer call, and a `VOID` record needs its cause fixed and `--new-pass`. Until the helper prints `AUDIT PASS` the transcript is **unaudited, not clean**: quote nothing from it. |
